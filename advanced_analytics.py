...
def _holding_durations(actions: pd.Series) -> dict:
    """
    Compute statistics on how long each position type (sell/hold/buy) is held
    consecutively.
    ...
    categorized = actions.apply(_categorize_action)
    current = categorized.iloc[0]
    count = 1
    for a in categorized.iloc[1:]:
        if a == current:
            count += 1
        else:
...
class EpisodeAdvancedAnalytics:
    ...
    def _compute_statistics(self, df: pd.DataFrame) -> dict:
        ...
        if "Action_Value" in df.columns:
            hold_stats = _holding_durations(df['Action_Value'])
            categorized = df['Action_Value'].apply(_categorize_action)
            stats = {
                ...
                "q_cash_std": float(df['Q_Cash'].std()) if 'Q_Cash' in df.columns else 0.0,
                "q_invest_std": float(df['Q_Invest'].std()) if 'Q_Invest' in df.columns else 0.0,
                "sell_action_count": int((categorized == "sell").sum()),
                "hold_action_count": int((categorized == "hold").sum()),
                "buy_action_count": int((categorized == "buy").sum()),
            }
        else:
            # Old format: binary actions
            hold_stats = _holding_durations(df['Action'])
            categorized = df['Action'].apply(lambda x: "buy" if x == 1 else "hold")
            stats = {
                "holding_duration_mean": hold_stats["mean"],
                "holding_duration_std": hold_stats["std"],
                "holding_duration_min": hold_stats["min"],
    ...
    def _save_advanced_plot(self, df: pd.DataFrame, summary: dict, episode: int) -> None:
        ...
        if "Action_Value" in df.columns:
            categorized = df['Action_Value'].apply(_categorize_action)
            action_array = df['Action_Value'].values.reshape(1, -1)
            imshow_vmin, imshow_vmax = -1, 1
            imshow_title = 'Action Timeline (Red=Sell, Yellow=Hold, Green=Buy)'
        else:
            categorized = df['Action'].apply(lambda x: "buy" if x == 1 else "hold")
            action_array = df['Action'].values.reshape(1, -1)
            imshow_vmin, imshow_vmax = 0, 1
            imshow_title = 'Action Timeline (Green=Buy, Red=Hold)'
        ...
        if buy_mask.any():
            ax1.scatter(df.loc[buy_mask, 'Step'], df.loc[buy_mask, 'Net_Worth'],
                        color='green', marker='^', s=60, zorder=5, label='Buy', alpha=0.7)
        if sell_mask.any():
            ax1.scatter(df.loc[sell_mask, 'Step'], df.loc[sell_mask, 'Net_Worth'],
                        color='red', marker='v', s=60, zorder=5, label='Sell', alpha=0.7)
        if hold_mask.any():
            ax1.scatter(df.loc[hold_mask, 'Step'], df.loc[hold_mask, 'Net_Worth'],
                        color='orange', marker='o', s=40, zorder=4, label='Hold', alpha=0.5)
        ...
        # 4. Action timeline (continuous values)
        ax4 = fig.add_subplot(gs[2, 0])
        ax4.imshow(action_array, aspect='auto', cmap='RdYlGn', vmin=imshow_vmin, vmax=imshow_vmax,
                   extent=[steps[0], steps[-1], 0, 1])
        ax4.set_title(imshow_title, fontweight='bold')
        ax4.set_xlabel('Step')
        ax4.set_yticks([])
