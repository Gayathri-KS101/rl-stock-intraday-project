"""
Trading Environment and Data Processing Module
...
def load_data(file_path):
    """
    Load and preprocess stock data with technical indicators and daily context.
    
    Args:
        file_path: Path to CSV file containing stock data
        
    Returns:
        DataFrame with added technical indicators and daily context features
    """
    ...
    df = df.merge(daily_df[['day_date', 'daily_sma_5', 'daily_sma_20', 'prev_close']], 
...
class StockTradingEnv(gym.Env):
    """
    Gymnasium environment for stock trading simulation.
    
    Features:
    - Multi-day intraday trading
    ...
    def __init__(self, df, initial_balance=10000, window_size=20):
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, 
                                           shape=(total_features,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        if action == 1:  # Buy
            ...
            # Logic: Sell ALL for now to simplify learning the "Exit" signal
            if self.shares_held > 0:
        ...
        # Reward Function
        # Change in Net Worth (The ultimate goal)
